...
conn.subscribe(destination='/queue/rnd', id=3, ack='auto')
#conn.send(body=' RND Team - add header', destination='/queue/rnd', headers={"rnd-member": "4", 'persistent': 'true', 'priority': '8' })
...
# Say, if you want to send the message "Hello Queue", you will run the code aspython queue.py "Hello Queue"

#conn.send(body=' RND- '.join(sys.argv[1:]), destination='/queue/queue-1', headers={"rnd-member": "4"})
#conn.send(body=' RND Team ', destination='/queue/rnd')

# When this message is received by the listener, it will be handled by the on_message method we defined above.
